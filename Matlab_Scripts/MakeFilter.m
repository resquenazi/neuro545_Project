...
% if fpar does not exist, the function will create parameters for you
...
if ~isfield(fpar, 'freq')
    fpar.freq=3; % number of cycles per degree
...
elseif strcmp(filterName, 'RadialCheckerboard')
    ...
    F_RAD=sin(RAD*fpar.freq.*2*pi-fpar.radPhase); %  concentric ring part of filter
    F_SPOKES=sin(THETA*fpar.spokes-fpar.spokePhase); % wedgy part of filter filter
    FILT=F_RAD.*F_SPOKES; % radial checkerboard filer
    FILT_INV=-FILT; % contrast reverse radial checkerboard filter
elseif strcmp(filterName, 'Checkerboard')
    F_X = sin(.5*2*pi*((X)));
    F_Y = sin(.5*2*pi*((Y)));
    FILT = F_X.*F_Y; % checkerboard filter
    FILT_INV=-FILT; % inverse checkerboard filter
else
    errordlg('fiterName not recognized');
end
if fpar.clip == 1 % binarize filter (i.e. no blurred edges)
    FILT=FILT>=0; 
    FILT_INV=FILT_INV>=0;
elseif fpar.clip == 2 % rectification of filter
    FILT(FILT<0) = 0;
    FILT_INV(FILT_INV<0) = 0;
else
    FILT = (FILT+1)./2; % do not binarize filter. results in gaussian blurred edges
...
if fpar.plot
    ...
    for f = 1:length(filterbank)
        ...
        title(['Filter ', fnames{f}])
        sgtitle('Contrast Reversed Filters');
    end
end
